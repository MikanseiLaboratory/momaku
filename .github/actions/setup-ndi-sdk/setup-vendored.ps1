...
if (-not (Test-Path -LiteralPath $hdr) -or -not (Test-Path -LiteralPath $lib)) {
    Write-Host "::error::Vendored NDI SDK missing. Expected $hdr and $lib. Run scripts/vendor-ndi-sdk-from-local.ps1 (see third_party/ndi-sdk-6/NOTICE)."
    exit 1
}
